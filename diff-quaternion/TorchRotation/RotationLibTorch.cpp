...
namespace RotationLibTorch
{
    ...
    {
        torch::Tensor length = torch::norm(q, 2, {-1}, true);
    ...
    torch::Tensor quat_to_rotvec(torch::Tensor q)
    {
        ...
        torch::Tensor xyz_norm = torch::norm(xyz, 2, {-1}, false);
    ...
    torch::Tensor quat_from_rotvec(torch::Tensor x)
    {
        ...
        torch::Tensor norms = torch::norm(x, 2, {-1}, false);
    ...
    std::vector<torch::Tensor> normalize_vec6d(torch::Tensor x)
    {
        ...
        last_col = last_col / torch::norm(last_col, 2, {-1}, true);
        second_col = torch::cross(-first_col, last_col, -1);
        second_col = second_col / torch::norm(second_col, 2, {-1}, true);
